...
func (r *AuthPostgres) CreateUser(user registration.User) (int, error) {
	var id int
	query := fmt.Sprintf("INSERT INTO %s (email, password_hash, enteredrefcode) values ($1, $2, $3) RETURNING id", usersTable)
	row := r.db.QueryRow(query, user.Email, user.Password, user.EnteredRefCode)
